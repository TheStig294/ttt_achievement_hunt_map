...
if SERVER then
    util.AddNetworkString("AHTestPersonalityDescription")
    util.AddNetworkString("AHRemoveTestPersonalityDescription")
    local nearTestEnts = {}
    ...
    local function ChooseFirstQuestion(ply, ent)
        ...
        startTestLever:EmitSound("ttt_achievement_hunt/custom_sounds/lofi.mp3")
        startTestLever:EmitSound("ttt_achievement_hunt/custom_sounds/lofi.mp3")
    ...
    local function EndTest(ply, ent, personality)
        SetGlobalBool("AHTestOver", true)
        -- Displaying the result to everyone nearby
        local personalityTable = AHTestPersonalities[personality]
        local desc = personalityTable.Description
        local plys = {}
        local message = "The " .. personalityTable.ID .. " type!"
        ...
        net.Start("AHTestPersonalityDescription")
        net.WriteString(desc)
        net.Send(plys)

        timer.Create("AHTestReset", #desc * 0.05 + 3, 1, function()
            net.Start("AHRemoveTestPersonalityDescription")
            net.Broadcast()
            -- Stops the music from looping and fades it out
            timer.Remove("AHTestSoundLoop")
            startTestLever:StopSound("ttt_achievement_hunt/custom_sounds/lofi.mp3")
            startTestLever:StopSound("ttt_achievement_hunt/custom_sounds/lofi.mp3")
            ...
            local propName = personalityTable.PropName
            ...
            TransformPlayerIntoProp(initPly, personalityTable.Prop)
        end)
    end
    ...
    hook.Add("PlayerUse", "AHTestInteraction", function(ply, ent)
        ...
        if name == "button_test_lever" then
            ...
            if not GetGlobalBool("AHTestIntroShown") then
                ...
                local welcomeMsgs = {"Welcome to a prop-sonaility test!", "Time to find out what kind of prop you are...", "Be true to yourself!", "Are you ready " .. initPly:Nick() .. "? Let's begin!"}
        ...
        elseif GetGlobalString("AHTestGivenAnswer", "noanswer") == "noanswer" and TestActive() then
            ...
            ent:EmitSound("ttt_achievement_hunt/custom_sounds/test_confirm.mp3")
            ...
            for personality, value in pairs(points) do
                local oldPoints = initPly.AHTestPoints[personality]

                if oldPoints then
                    oldPoints = oldPoints + value
                else
                    initPly.AHTestPoints[personality] = value
                end
            end
            ...
            timer.Create("AHTestLoadNextQuestion", 0.3, 1, function()
                ...
                if questionNumber >= 8 then
                    ...
                    local personality = table.GetWinningKey(initPly.AHTestPoints)

                    for _, nearEnt in ipairs(nearTestEnts) do
                        if not IsPlayer(nearEnt) then continue end
                        PrintCentreShadowed(nearEnt, "The results are in, you might be...")
                        nearEnt:PrintMessage(HUD_PRINTTALK, "The results are in, you might be...")

                        timer.Create("AHTestEnd", 2, 1, function()
                            EndTest(ply, ent, personality)
                        end)
                    end
                else
...
if CLIENT then
    ...
    hook.Add("PostDrawHUD", "AHTestUI", function()
        ...
        textWidth, textHeight = draw.WordBox(16, questionXPos, questionYPos, questionText, "AHQuizFont", boxColour, COLOR_WHITE, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
        ...
        for index, answerText in ipairs(Q.AnswerText) do
            ...
            textWidth, textHeight = draw.WordBox(16, answerXPos, answerYPos, answerText, "AHQuizFont", boxColour, COLOR_WHITE, textAlign, TEXT_ALIGN_CENTER)
        end
        ...
        if currentEntPos ~= lastEntPos then
            surface.PlaySound("ttt_achievement_hunt/custom_sounds/test_text_scroll_beep.mp3")
        end
    ...
    net.Receive("AHTestPersonalityDescription", function()
        ...
        hook.Add("DrawOverlay", "AHDrawPersonalityDescription", function()
            draw.DrawText(descDisplayed, "AHQuizFont", halfScreenWidth, halfScreenHeight, COLOR_WHITE, TEXT_ALIGN_CENTER)
        end)
    end)

    net.Receive("AHRemoveTestPersonalityDescription", function()
        hook.Remove("DrawOverlay", "AHDrawPersonalityDescription")
    end)

    -- Remove the personality test text if the round ends
    hook.Add("TTTPrepareRound", "AHRemoveTestPersonalityDescription", function()
        hook.Remove("DrawOverlay", "AHDrawPersonalityDescription")
    end)
end